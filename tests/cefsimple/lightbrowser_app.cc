...
#include "tests/cefsimple/lightbrowser_app.h"
...
LightBrowserApp::LightBrowserApp() = default;

void LightBrowserApp::OnContextInitialized() {
  ...
  // LightBrowserHandler implements browser-level callbacks.
  CefRefPtr<LightBrowserHandler> handler(new LightBrowserHandler(use_alloy_style));
  ...
  if (use_views) {
    ...
    // CreateWindowEx().
    window_info.SetAsPopup(nullptr, "lightbrowser");
#endif
...
CefRefPtr<CefClient> LightBrowserApp::GetDefaultClient() {
  // Called when a new browser window is created via Chrome style UI.
  return LightBrowserHandler::GetInstance();
}
