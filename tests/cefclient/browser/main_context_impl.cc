...
// The default NTP filename to load (relative to the app working directory).
const char kDefaultNtpFilename[] = "ntp.html";
...
MainContextImpl::MainContextImpl(CefRefPtr<CefCommandLine> command_line,
  ...
  // Force Chrome Runtime.
  command_line_->AppendSwitch("enable-chrome-runtime");
  ...
  // Atmos: Default to Chrome runtime with Views (full tab bar + omnibox).
  // Only allow override to disable Views when explicitly using native parent
  // or windowless rendering.
  const bool use_chrome_native_parent =
      command_line->HasSwitch(switches::kUseNative);

  if (use_windowless_rendering_) {
    // Windowless rendering always uses Alloy style native window.
    use_views_ = false;
  } else if (use_chrome_native_parent) {
    // Native parent: allow, Views not needed.
    use_views_ = false;
  } else {
    // Default: use Chrome runtime with Views for full tab/omnibox UI.
    use_views_ = true;
  }


  // Whether Alloy style will be used.
  // Atmos: Force Chrome style (Alloy = false) unless OSR requires it.
...
std::string MainContextImpl::GetMainURL(
  ...
  if (command_line->HasSwitch(switches::kUrl)) {
    return command_line->GetSwitchValue(switches::kUrl);
  }
  ...
  if (use_views_ && command_line->HasSwitch(switches::kHideFrame)) {
    ...
    return test_runner::GetTestURL("draggable");
  }

  // Build an absolute file:// URL from the app working directory.
  // This ensures ntp.html is found next to the executable.
  const std::string working_dir = GetAppWorkingDirectory();
  if (!working_dir.empty()) {
    // Convert Windows backslash path to file:// URL format.
    std::string path = working_dir + kDefaultNtpFilename;
    // Replace backslashes with forward slashes.
    std::replace(path.begin(), path.end(), '\\', '/');
    // Ensure we have three slashes: file:///C:/path/ntp.html
    return "file:///" + path;
  }

  // Fallback if working directory is unavailable.
  return std::string("file:///") + kDefaultNtpFilename;
}
